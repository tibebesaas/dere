...
private:
    ...
    void trimTrailingSpace(std::ostringstream& out) {
        std::string result = out.str();  // Get the string content from the stringstream
        if (!result.empty() && result[result.size() - 1] == ' ') {
            result = result.substr(0, result.size() - 1);  // Remove the last space
        }
        out.str("");  // Clear the stringstream
        out.clear();  // Reset the error flags
        out << result;  // Write the trimmed result back into the stringstream
    }
};
